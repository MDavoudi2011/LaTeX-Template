% ==============================================================================
% styles/environments.tex
% ==============================================================================

\tcbuselibrary{skins, breakable, theorems}

% استایل ۱: باکس با هدر روبان شکل (Ribbon Box) برای قضیه، تعریف و ...
\makeatletter
\tcbset{
	ribbon_box/.style={
		enhanced,
		breakable,
		colback=white,
		attach boxed title to top right={yshift*=-\tcboxedtitleheight},
		fonttitle=\bfseries\setRTL,
		before upper=\setRTL,
		boxed title size=title,
		boxed title style={
			sharp corners,
			rounded corners=northeast,
			colback=tcbcolframe,
			boxrule=0pt,
		},
		underlay boxed title={
			\path[fill=tcbcolframe] (title.south east)--(title.south west)
			to[out=180, in=0] ([xshift=-5mm]title.west)--
			(title.center-|frame.west)
			[rounded corners=\kvtcb@arc] |-
			(frame.north) -| cycle;
		},
	}
}
\makeatother

% استایل ۲: باکس مینیمال با خط کناری (Side Box) برای مثال، حل، نکته و ...
\tcbset{
	side_box/.style={
		enhanced,
		breakable,
		frame hidden,
		boxrule = 0sp,
		borderline east = {2pt}{0pt}{#1}, % خط کناری در سمت راست
		sharp corners,
		detach title,
		before upper = \setRTL\tcbtitle\par\smallskip,
		coltitle = #1,
		fonttitle = \bfseries\setRTL,
		separator sign none,
		segmentation style={solid, #1},
	}
}

% استایل ۳: باکس با هدر روی ضلع بالا (Top Box) برای هشدار و ...
\tcbset{
    top_box/.style={
        breakable,
        enhanced,
        colback=white,
        arc=0pt,
        outer arc=0pt,
        fonttitle=\bfseries\large\setRTL,
        before upper=\setRTL,
        attach boxed title to top right={},
        boxed title style={
            enhanced,
            skin=enhancedfirst jigsaw,
            arc=3pt,
            bottom=0pt,
            interior style={fill=tcbcolframe}
        },
    }
}

% شمارنده مشترک برای همه محیط‌های ریاضی که وابسته به فصل است
\newcounter{mathbox}[chapter]
% تنظیم نمایش شماره به صورت X.Y (مثلاً 1.2)
\renewcommand{\themathbox}{\thechapter.\arabic{mathbox}}

% ------------------------------------------------------------------------------
% محیط‌های شماره‌دار
% ------------------------------------------------------------------------------

% 1. قضیه
\NewTColorBox[use counter=mathbox]{theorem}{ o }{
    ribbon_box,
    colframe=BoxBlueFrame,
    title={\faLightbulb \quad قضیه \themathbox\IfValueT{#1}{ \textnormal{(#1)}}}
}

% 2. لم
\NewTColorBox[use counter=mathbox]{lemma}{ o }{
    ribbon_box,
    colframe=BoxBlueFrame,
    title={\faLink \quad لم \themathbox\IfValueT{#1}{ \textnormal{(#1)}}}
}

% 3. نتیجه
\NewTColorBox[use counter=mathbox]{corollary}{ o }{
    ribbon_box,
    colframe=BoxBlueFrame,
    title={\faProjectDiagram \quad نتیجه \themathbox\IfValueT{#1}{ \textnormal{(#1)}}}
}

% 4. تعریف
\NewTColorBox[use counter=mathbox]{definition}{ o }{
    ribbon_box,
    colframe=BoxGreenFrame,
    title={\faBook \quad تعریف \themathbox\IfValueT{#1}{ \textnormal{(#1)}}}
}

% 5. سوال
\NewTColorBox[use counter=mathbox]{question}{ o }{
    ribbon_box,
    colframe=BoxGrayFrame!80!black,
    title={\faQuestionCircle \quad سوال \themathbox\IfValueT{#1}{ \textnormal{(#1)}}}
}

% 6. مثال
\NewTColorBox[use counter=mathbox]{example}{ o }{
    side_box=BoxGrayFrame,
    colback=BoxGray,
    title={\faCogs \quad مثال \themathbox\IfValueT{#1}{ \textnormal{(#1)}}}
}

% ------------------------------------------------------------------------------
% محیط‌های بدون شماره
% ------------------------------------------------------------------------------

% 7. حل
\NewTColorBox{solution}{}{
    side_box=BoxGreenFrame,
    colback=BoxGreen,
    title={\faCheckCircle \quad حل}
}

% 8. نکته مهم
\NewTColorBox{note}{ o }{
    side_box=BoxYellowFrame,
    colback=BoxYellow,
    title={\faStar \quad نکته مهم\IfValueT{#1}{ \textnormal{(#1)}}}
}

% 9. هشدار (استفاده از استایل Top Box)
\NewTColorBox{warning}{ o }{
    top_box,
    colframe=BoxRedFrame,
    colbacktitle=BoxRedFrame,
    title={\faExclamationTriangle \quad هشدار\IfValueT{#1}{ \textnormal{(#1)}}}
}

% ------------------------------------------------------------------------------
% محیط اثبات (Proof)
% ------------------------------------------------------------------------------
\renewcommand{\qedsymbol}{$\blacksquare$}
\renewenvironment{proof}{{\bfseries \textcolor{BoxBlueFrame!90!black}{\faPenFancy \quad اثبات.}\quad}}{\hfill\qedsymbol\newline}
